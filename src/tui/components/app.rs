...
/// Per-tab content elements for the main tabs, built once per frame and
/// assembled into `TabItem`s by `App::build_tab_items`.
struct TabContents {
    scores: Element,
    standings: Element,
    settings: Element,
...
impl App {
    ...
    fn render_main_tabs_with_states(
        ...
        let active_key = Self::active_tab_key(state.navigation.current_tab);

        // Determine content for active tab - if document is open, show document instead
        let contents = if let Some(doc_entry) = state.navigation.document_stack.last() {
            self.build_document_tab_contents(state, doc_entry)
        } else {
            self.build_active_tab_contents(state, component_states)
        };

        let tabs = Self::build_tab_items(contents);
    ...
    /// String key TabbedPanel uses to identify the currently active tab
    fn active_tab_key(current_tab: Tab) -> &'static str {
        match current_tab {
            Tab::Scores => "scores",
    ...
    /// A document is open: render it (with breadcrumb) into the active tab's
    /// content slot; the other tabs render as empty since TabbedPanel discards them.
    fn build_document_tab_contents(
        &self,
        state: &AppState,
        ...
        let mut contents = TabContents::empty();
        match state.navigation.current_tab {
            Tab::Scores => contents.scores = content_with_breadcrumb,
            Tab::Standings => contents.standings = content_with_breadcrumb,
            Tab::Settings => contents.settings = content_with_breadcrumb,
            #[cfg(feature = "development")]
            Tab::Demo => contents.demo = content_with_breadcrumb,
        }
        contents
    }

    /// No document open - render only the active tab's content. The other tabs are hidden by
    ...
    fn build_active_tab_contents(
        ...
        match state.navigation.current_tab {
            ...
            Tab::Demo => {
                ...
                contents.demo = DemoTab.view(&demo_props, demo_state);
            }
        }
        contents
    }

    /// Assemble the final `TabItem` list from per-tab content elements
    fn build_tab_items(contents: TabContents) -> Vec<TabItem> {
        #[cfg(feature = "development")]
        let tabs = vec![
            TabItem::new("scores", "Scores", contents.scores),
            TabItem::new("standings", "Standings", contents.standings),
            TabItem::new("settings", "Settings", contents.settings),
            TabItem::new("demo", "Demo", contents.demo),
        ];
