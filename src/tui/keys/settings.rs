...
/// Handle keys while the settings modal (confirm flow for a selected setting) is open.
///
/// Note: no Esc arm here - ESC is intercepted globally by handle_esc_key's priority-2
/// check (settings modal open) before key_to_action ever reaches this function, so an
/// Esc arm here would be dead code.
fn handle_settings_modal_keys(key: KeyEvent) -> Option<Action> {
...
/// Left/Right always navigate categories. Category selection is component-local
/// (SettingsTabState::selected_category), so the message carries the Config that
/// `update()` needs to rebuild doc_nav for the new category - the same pattern
/// ActivateSetting in `handle_settings_tab_keys` already uses.
fn handle_settings_category_nav_keys(key: KeyEvent, state: &AppState) -> Option<Action> {
...
pub fn handle_settings_tab_keys(
    ...
    if is_settings_modal_open(state, component_states) {
        return handle_settings_modal_keys(key);
    }

    // No modal open - handle normal navigation
    if let Some(action) = handle_settings_category_nav_keys(key, state) {
        return Some(action);
    }
